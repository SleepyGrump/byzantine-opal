...
2. Run all the commands you want from "1. commands". (They're generally good for all games so no reason not to install them all. You can always disable later if you find one you don't want on your game.) Commands are generally grouped together with others that rely on one another.
