...
Code functions:
	...
	fetch(table, columns, where query, row delimiter, column delimiter, offset)
		...
		row delimiter: defaults to @@ROW@@
		column delimiter: defaults to @@COLUMN@@
	...
	post(table, columns, values, where query, column delimiter, value delimiter)
		table, columns, values, and where query are REQUIRED.
		column delimiter: defaults to @@COLUMN@@
		value delimiter: defaults to column delimiter
...
@force me=&vq [v(d.sf)]=[v(d.sd)]
@tel [v(d.sd)]=[v(d.sf)]
@tel [v(d.sf)]=[v(d.sc)]

&d.default-row-delimiter [v(d.sd)]=@@ROW@@

&d.default-column-delimiter [v(d.sd)]=@@COLUMN@@
...
@@ %3: row delimiter: defaults to @@ROW@@
@@ %4: column delimiter: defaults to @@COLUMN@@
...
@@ %4: column delimiter
@@ %5: value delimiter
&f.globalpp.post [v(d.sf)]=strcat(setq(C, if(t(strlen(%4)), %4, v(d.default-column-delimiter))), setq(V, if(t(strlen(%5)), %5, %qC)), setq(R, v(d.default-row-delimiter)), case(0, cor(isstaff(%#), cand(not(member(num(me), %@)), hastype(%@, THING), andflags(%@, I!h!n), isstaff(owner(%@)))), #-1 PERMISSION DENIED, lte(words(%0), 1), #-1 TOO MANY TABLES (%0), t(%0), #-1 NO TABLE SPECIFIED, member(fetch(), %0, %qR), #-1 TABLE '%0' NOT FOUND, t(%1), #-1 NO COLUMN SPECIFIED, not(t(setr(D, setdiff(%1, first(fetch(%0), %qR), %qC)))), #-1 COLUMNS '%qD' NOT FOUND, t(%2), #-1 NO VALUE SPECIFIED, eq(words(%1, %qC), words(%2, %qV)), #-1 NUMBER OF COLUMNS AND VALUES MUST MATCH, t(%3), #-1 NO WHERE CRITERIA PROVIDED, member(INSERT, %3), case(0, strmatch(%3, *=*), #-1 WHERE QUERY NOT SPECIFIC ENOUGH, strcat(setq(E, fetch(%0,, %3)), case(1, gt(words(%qE, %qR), 2), #-1 UPDATE WOULD AFFECT TOO MANY ROWS, lte(words(rest(%qE, %qR), %qC), 1), #-1 UPDATE WOULD AFFECT ZERO ROWS, strcat(setq(F, 1), setr(O, ulocal(f.update-table, %0, %1, %2, %3, %qC, %qV)))))), strcat(setq(F, 1), setr(O, ulocal(f.insert-table, %0, %1, %2, %3, %qC, %qV)))), if(cand(%qF, eq(strlen(%qO), 0)), success))

@@ %0: table list
@@ %1: viewer
&f.filter-unlocked-tables [v(d.sf)]=if(isstaff(%1), %0, setinter(%0, v(d.unlocked_tables), v(d.default-row-delimiter)))

@@ %0: table list
&f.filter-hidden-tables [v(d.sf)]=setdiff(%0, v(d.hidden_tables), v(d.default-row-delimiter))

@@ %0: table list
@@ %1: user viewing this
&layout.show_tables [v(d.sf)]=strcat(header(All available tables, %1), %r, formatcolumns(%0, v(d.default-row-delimiter), %1), %r, footer(+db <table name> for more, %1))

&f.flip-data-horizontal [v(d.sf)]=strcat(setq(R, v(d.default-row-delimiter)), setq(C, v(d.default-column-delimiter)), setq(0, setq(1,)), null(iter(first(%0, %qR), setq(1, strcat(if(gt(strlen(itext(0)), %q0), setq(0, strlen(itext(0)))), %q1, %qR, ansi(first(themecolors()), itext(0)), :, %qR, extract(rest(%0, %qR), inum(0), 1, %qC))), %qC, @@)), trim(%q1, b, %qR), @@WIDTH@@, %q0)

&layout.single_row [v(d.sf)]=strcat(setq(0, ulocal(f.flip-data-horizontal, %1)), setq(1, last(%q0, @@WIDTH@@)), setq(0, before(%q0, @@WIDTH@@)), header(Single result from the '%0' table', %2), %r, multicol(%q0, inc(%q1) *,, v(d.default-row-delimiter), %2), %r, footer(Only one result returned., %2))

@@ %0: table name
@@ %1: table contents
@@ %2: user viewing this
&layout.page-footer [v(d.sf)]=strcat(Results, %b, setq(P, default(%2/_current.page.%0, 0)), inc(%qP), %b, -, %b, setr(L, min(add(%qP, 25), add(%qP, dec(words(%1, v(d.default-row-delimiter)))))), if(eq(%qL, 25), strcat(%,%b, +db/n for next page), if(gt(%qP, 0), strcat(%,%b, +db/p for previous page))))

&layout.table [v(d.sf)]=if(gt(words(%1, v(d.default-row-delimiter)), 2), strcat(setq(W, v(d.column_widths.%0)), header(From the '%0' table, %2), %r, if(t(%qW), multicol(edit(%1, v(d.default-column-delimiter), v(d.default-row-delimiter)), %qW, 1, v(d.default-row-delimiter), %2), formatdb(%1, 1,,, %2)), %r, footer(ulocal(layout.page-footer, %0, %1, %2), %2)), ulocal(layout.single_row, %0, %1, %2))

@@ %0: table name
@@ %1: table contents
@@ %2: user viewing this
&layout.table-columns [v(d.sf)]=strcat(setq(R, v(d.default-row-delimiter)), setq(C, v(d.default-column-delimiter)), header(Columns in the the '%0' table, %2), %r, formattable(iter(first(%1, %qR), squish(strcat(itext(0), :, %qR, setq(I, sql(ulocal(sql.get-single-column-info, %0, itext(0)), %qR, %qC)), if(first(%qI, %qC), nullable, required), %b, switch(extract(%qI, 2, 1, %qC), PRI, primary key, UNI, unique, MUL, multi-key,), %b, extract(%qI, 3, 1, %qC), %b, extract(%qI, 4, 1, %qC), %b, extract(%qI, 5, 1, %qC), %b, extract(%qI, 6, 1, %qC))), %qC, %qR), 2,, %qR, %2), %r, footer(, %2))
...
&c.+db_table [v(d.sc)]=$+db *:@assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%0'.; }; @pemit %#=ulocal(layout.table, %0, fetch(%0, v(d.columns.%0)), %#); &_current.table %#=%0; @wipe %#/_current.page.*; @wipe %#/_current.search.*; &_current.page.%0 %#=0;

&c.+db_next [v(d.sc)]=$+db/n*:@break strmatch(%0, * *); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%q0'.; }; @eval if(eq(words(trim(setr(1, fetch(%q0, v(d.columns.%q0), xget(%#, _current.search.%q0),,, setr(2, add(default(%#/_current.page.%q0, 0), 25)))), b, v(d.default-row-delimiter)), v(d.default-row-delimiter)), 1), strcat(setq(1, fetch(%q0, v(d.columns.%q0), xget(%#, _current.search.%q0),,, 0)), setq(2, 0), setq(3, 1))); &_current.page.%q0 %#=%q2; @pemit %#=ulocal(layout.table, %q0, %q1, %#); @break t(%q3)={  @trigger me/tr.message=%#, Reached the end of the list. Starting over.; };
...
&c.+db_rand_table [v(d.sc)]=$+db/rand *:@assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%0'.; }; @pemit %#=ulocal(layout.table, %0, fetch(%0, v(d.columns.%0), RAND), %#); &_current.table %#=%0;

&c.+db_rand [v(d.sc)]=$+db/rand:@assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%q0'.; }; @pemit %#=ulocal(layout.table, %q0, fetch(%q0, v(d.columns.%q0), RAND), %#);

&c.+db_find_table [v(d.sc)]=$+db/find */*=*:@assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%0'.; }; @break t(setr(U, setdiff(%1, trim(fetch(%0,, 1=0), r, v(d.default-row-delimiter)), v(d.default-column-delimiter))))={ @trigger me/tr.error=%#, Could not find a column named '%qU'.;}; @pemit %#=ulocal(layout.table, %0, fetch(%0, v(d.columns.%0), setr(S, strcat(LOWER\(CONVERT\(%1 USING utf8mb4\)\), %b, LIKE, %b', setr(A, sanitize(%2)), *'))), %#); &_current.table %#=%0; @wipe %#/_current.page.*; @wipe %#/_current.search.*; &_current.page.%0 %#=0; &_current.search.%0 %#=%qS;

&c.+db_find [v(d.sc)]=$+db/find *=*:@break strmatch(%0, */*); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%q0'.; }; @break t(setr(U, setdiff(%0, trim(fetch(%q0,, 1=0), r, v(d.default-row-delimiter)), v(d.default-column-delimiter))))={ @trigger me/tr.error=%#, Could not find a column named '%qU'.;}; @pemit %#=ulocal(layout.table, %q0, fetch(%q0, v(d.columns.%q0), setr(S, strcat(LOWER\(CONVERT\(%0 USING utf8mb4\)\), %b, LIKE, %b', setr(A, sanitize(%1)), *'))), %#); @wipe %#/_current.page.*; @wipe %#/_current.search.*; &_current.page.%q0 %#=0; &_current.search.%q0 %#=%qS;


&c.+db_columns_table [v(d.sc)]=$+db/c* *:@break strmatch(%1, *=*); @break strmatch(%1, * *); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), %1, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%1'.; }; @pemit %#=ulocal(layout.table-columns, %1, fetch(%1,, 1=0), %#); &_current.table %#=%1;

&c.+db_columns [v(d.sc)]=$+db/c*:@break strmatch(%0, * *); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%q0'.; }; @pemit %#=ulocal(layout.table-columns, %q0, fetch(%q0,, 1=0), %#);

&c.+db_columns_set_table [v(d.sc)]=$+db/c* *=*:@break strmatch(%0, *w*); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), %1, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%1'.; }; @break t(setr(U, setdiff(edit(%2, %b, v(d.default-column-delimiter)), trim(fetch(%1,, 1=0), r, v(d.default-row-delimiter)), v(d.default-column-delimiter))))={ @trigger me/tr.error=%#, Could not find all the columns in your list. Can't find: '[itemize(%qU, v(d.default-column-delimiter))]';}; &d.columns.%1 %vQ=%2; @trigger me/tr.success=%#, The column list for '%1' is now: %2; @assert cor(not(t(setr(W, v(d.column_widths.%1)))), eq(words(default(d.columns.%1, edit(first(fetch(%1,, 1=0), v(d.default-row-delimiter)), v(d.default-column-delimiter), %b))), words(%qW)))={ &d.column_widths.%1 %vQ=; @trigger me/tr.message=%#, Your column widths no longer match your number of columns. This can lead to data leakage. Because of this%, your custom column widths have been wiped. They were: %qW; }; &_current.table %#=%1;

&c.+db_columns_set [v(d.sc)]=$+db/c* *:@break strmatch(%0, *w*); @break strmatch(%1, *=*); @break eq(words(%1), 1); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%q0'.; }; @break t(setr(U, setdiff(edit(%1, %b, v(d.default-column-delimiter)), trim(fetch(%q0,, 1=0), r, v(d.default-row-delimiter)), v(d.default-column-delimiter))))={ @trigger me/tr.error=%#, Could not find all the columns in your list. Can't find: '[itemize(%qU, v(d.default-column-delimiter))]';}; &d.columns.%q0 %vQ=%1; @trigger me/tr.success=%#, The column list for '%q0' is now: %1; @assert cor(not(t(setr(W, v(d.column_widths.%q0)))), eq(words(default(d.columns.%q0, edit(first(fetch(%q0,, 1=0), v(d.default-row-delimiter)), v(d.default-column-delimiter), %b))), words(%qW)))={ &d.column_widths.%q0 %vQ=; @trigger me/tr.message=%#, Your column widths no longer match your number of columns. This can lead to data leakage. Because of this%, your custom column widths have been wiped. They were: %qW; };

&c.+db_columns_widths_table [v(d.sc)]=$+db/c* *=*:@assert strmatch(%0, *w*); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), %1, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%1'.; }; @assert eq(words(default(d.columns.%1, edit(first(fetch(%1,, 1=0), v(d.default-row-delimiter)), v(d.default-column-delimiter), %b))), words(%2))={ @trigger me/tr.error=%#, You must have the same number of column widths as columns.; }; &d.column_widths.%1 %vQ=%2; @trigger me/tr.success=%#, The column width list for '%1' is now: %2; &_current.table %#=%1;

&c.+db_columns_widths [v(d.sc)]=$+db/c* *:@assert strmatch(%0, *w*); @break strmatch(%q0, *=*); @assert member(ulocal(f.filter-unlocked-tables, fetch(), %#), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Cannot find a table named '%q0'.; }; @assert eq(words(default(d.columns.%q0, edit(first(fetch(%q0,, 1=0), v(d.default-row-delimiter)), v(d.default-column-delimiter), %b))), words(%1))={ @trigger me/tr.error=%#, You must have the same number of column widths as columns.; }; &d.column_widths.%q0 %vQ=%1; @trigger me/tr.success=%#, The column width list for '%q0' is now: %1;

&c.+db/hide_table [v(d.sc)]=$+db/hide *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%0'.; }; @break member(v(d.hidden_tables), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%0' is already hidden.; }; &d.hidden_tables %vQ=[setunion(v(d.hidden_tables), %0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%0' has been added to the list of hidden tables. You will no longer see it in +db.; &_current.table %#=;

&c.+db/hide [v(d.sc)]=$+db/hide:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%q0'.; }; @break member(v(d.hidden_tables), %q0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%q0' is already hidden.; }; &d.hidden_tables %vQ=[setunion(v(d.hidden_tables), %q0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%q0' has been added to the list of hidden tables. You will no longer see it in +db.; &_current.table %#=;

&c.+db/show_table [v(d.sc)]=$+db/show *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%0'.; }; @assert member(v(d.hidden_tables), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%0' is not currently hidden.; }; &d.hidden_tables %vQ=[setdiff(v(d.hidden_tables), %0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%0' has been unhidden. You will now see it in +db.; &_current.table %#=%0;

&c.+db/show [v(d.sc)]=$+db/show:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%q0'.; }; @assert member(v(d.hidden_tables), %q0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%q0' is not currently hidden.; }; &d.hidden_tables %vQ=[setdiff(v(d.hidden_tables), %q0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%q0' has been unhidden. You will now see it in +db.;

&c.+db/unlock_table [v(d.sc)]=$+db/unlock *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%0'.; }; @break member(v(d.unlocked_tables), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%0' is already unlocked and visible to players.; }; &d.unlocked_tables %vQ=[setunion(v(d.unlocked_tables), %0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%0' has been unlocked. Players can now see it in +db.; &_current.table %#=%0;

@@ Skipping &c.+db/unlock with no table specified because we don't want to risk unsecuring something by accident.

&c.+db/lock_table [v(d.sc)]=$+db/lock *:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%0'.; }; @assert member(v(d.unlocked_tables), %0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%0' is locked and is not currently visible to players.; }; &d.unlocked_tables %vQ=[setdiff(v(d.unlocked_tables), %0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%0' has been locked. Players can no longer see it in +db.; &_current.table %#=%0;

&c.+db/lock [v(d.sc)]=$+db/lock:@assert isstaff(%#)={ @trigger me/tr.error=%#, You must be staff to use this function.; }; @assert member(fetch(), setr(0, xget(%#, _current.table)), v(d.default-row-delimiter))={ @trigger me/tr.error=%#, Can't find a table named '%q0'.; }; @assert member(v(d.unlocked_tables), %q0, v(d.default-row-delimiter))={ @trigger me/tr.error=%#, The table '%q0' is locked and is not currently visible to players.; }; &d.unlocked_tables %vQ=[setdiff(v(d.unlocked_tables), %q0, v(d.default-row-delimiter))]; @trigger me/tr.success=%#, The table '%q0' has been locked. Players can no longer see it in +db.;
