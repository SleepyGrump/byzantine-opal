...
All requests broadcasts would get sent to both the Jobs channel and the Request Jobs channel. That way you can monitor buckets individually or all at once. (Or both, but that's a lot of spam.)
*/

&BROADCAST_CHANNEL [v(JOB_VA)]=Jobs

&TRIG_BROADCAST [v(JOB_VA)]=@cemit [setq(0, parent(%0))][setq(0, ifelse(isdbref(%q0), %q0, %vc))][v(BROADCAST_CHANNEL)]=[name(%q0)]: [u(%q0/BLETTER_%2, %0, %1, %3, %4)]; @if hasattr(%q0, BROADCAST_CHANNEL)={ @cemit xget(%q0, BROADCAST_CHANNEL)=[u(%q0/BLETTER_%2, %0, %1, %3, %4)]; };
