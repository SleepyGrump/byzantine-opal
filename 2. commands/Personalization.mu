...
&f.grab-matching-object [v(d.bf)]=case(1, strmatch(%0, me), %1, strmatch(%0, here), loc(%1), cand(isstaff(%1), isdbref(%0)), %0, isdbref(%0), if(member(ulocal(f.available-objects-to-%2, %1), %0), %0, #-1 NOT FOUND), t(setr(M, grab(iter(ulocal(f.available-objects-to-%2, %1), strcat(name(itext(0)), ~, itext(0)),, v(d.default-row-delimiter)), %0*, v(d.default-row-delimiter)))), rest(%qM, ~), #-1 NOT FOUND)
...
&f.grab-matching-settable-object [v(d.bf)]=case(1, strmatch(%0, me), %1, cand(strmatch(%0, here), cor(isstaff(%1), isowner(loc(%0), %1))), loc(%1), cand(isstaff(%1), isdbref(%0)), %0, isdbref(%0), if(member(ulocal(f.available-objects-to-set-%2s, %1), %0), %0, #-1 NOT FOUND), t(setr(M, grab(iter(ulocal(f.available-objects-to-set-%2s, %1), strcat(name(itext(0)), ~, itext(0)),, v(d.default-row-delimiter)), %0*, v(d.default-row-delimiter)))), rest(%qM, ~), #-1 NOT FOUND)
...
&c.+view [v(d.bc)]=$+view:@assert t(setr(V, filter(filter.has-views, ulocal(f.available-objects-to-view, %#))))={ @trigger me/tr.error=%#, There are no objects with +views in your area.; }; @assert gt(words(%qV), 1)={ @force %#=+view %qV; }; @trigger me/tr.message=%#, The following objects have +views on them: [itemize(ulocal(f.objects-with-views, %qV), v(d.default-row-delimiter))];

&c.+view_item [v(d.bc)]=$+view *:@break strmatch(%0, */*); @assert t(setr(O, ulocal(f.grab-matching-object, %0, %#, view)))={ @break eq(words(ulocal(f.grab-matching-object,, %#, view)), 1)={ @force %#=+view /%0; }; @trigger me/tr.error=%#, Could not find an object called '%0'.; }; @assert t(setr(V, getpair(%qO, view-)))={ @trigger me/tr.error=%#, moniker(%qO) does not have +views.; }; @assert gt(words(%qV, v(d.default-row-delimiter)), 1)={ @force %#=+view %qO/[rest(%qV, v(d.default-column-delimiter))]; }; @trigger me/tr.message=%#, moniker(%qO) has the following +views: [itemize(iter(%qV, rest(itext(0), v(d.default-column-delimiter)), v(d.default-row-delimiter), v(d.default-row-delimiter)), v(d.default-row-delimiter))];

&c.+view_item_view [v(d.bc)]=$+view */*:@assert t(setr(O, ulocal(f.grab-matching-object, %0, %#, view)))={ @trigger me/tr.error=%#, Could not find an object called '%0'.; }; @assert t(setr(V, getpair(%qO, view-, %1)))={ @trigger me/tr.error=%#, moniker(%qO) does not have a +view matching '%1'.; }; @break strmatch(%qV, view-*[v(d.default-column-delimiter)]*[v(d.default-row-delimiter)]*)={ @trigger me/tr.message=%#, moniker(%qO) has the following +views: [itemize(iter(%qV, rest(itext(0), v(d.default-column-delimiter)), v(d.default-row-delimiter), v(d.default-row-delimiter)), v(d.default-row-delimiter))]; }; @break strmatch(%qV, view-*[v(d.default-column-delimiter)]*)={ @force %#=+view %qO/[rest(%qV, v(d.default-column-delimiter))]; }; @pemit %#=ulocal(layout.view, %qO, %1, %qV, %#);
...
&layout.note_title [v(d.bf)]=strcat(if(t(%2), %2%b), moniker(%0)'s, %b, if(strmatch(setr(0, getpairkey(%0, _note-, %1)), *[v(d.default-column-delimiter)]*),, %q0), %b, note)
...
&layout.note [v(d.bf)]=strcat(header(ulocal(layout.note_title, %0, %1, %4), %3), %r, formattext(ulocal(%0/[first(%2, v(d.default-column-delimiter))]), 0, %3), %r, formattext(strcat(%r, if(t(setr(A, rest(default(%0/_notesettings-[getpairattr(%0, _note-, %1)], 0|), |))), %cgApproved%cn: %qA, %crUnapproved%cn: Staff has not approved this +note.)), 0, %3),, %r, footer(ulocal(layout.note_footer, %0, %1), %3))
...
&c.+note [v(d.bc)]=$+note:@assert t(setr(V, filter(filter.has-notes, ulocal(f.available-objects-to-note, %#),,, %#)))={ @trigger me/tr.error=%#, There are no objects with +notes in your area.; }; @assert gt(words(%qV), 1)={ @force %#=+note %qV; }; @trigger me/tr.message=%#, The following objects have +notes on them: [itemize(ulocal(f.objects-with-notes, %qV), v(d.default-row-delimiter))];

&c.+note_item [v(d.bc)]=$+note *:@break strmatch(%0, */*); @assert t(setr(O, ulocal(f.grab-matching-object, %0, %#, note)))={ @trigger me/tr.error=%#, Could not find an object called '%0'.; }; @assert t(setr(V, ulocal(f.get-visible-notes, %qO,, %#)))={ @trigger me/tr.error=%#, moniker(%qO) does not have +notes.; }; @assert gt(words(%qV, v(d.default-row-delimiter)), 1)={ @force %#=+note %qO/[rest(%qV, v(d.default-column-delimiter))]; }; @trigger me/tr.message=%#, moniker(%qO) has the following +notes: [itemize(iter(%qV, rest(itext(0), v(d.default-column-delimiter)), v(d.default-row-delimiter), v(d.default-row-delimiter)), v(d.default-row-delimiter))];

&c.+note_item_note [v(d.bc)]=$+note */*:@assert t(setr(O, ulocal(f.grab-matching-object, %0, %#, note)))={ @trigger me/tr.error=%#, Could not find an object called '%0'.; }; @assert t(setr(V, ulocal(f.get-visible-notes, %qO, %1, %#)))={ @trigger me/tr.error=%#, moniker(%qO) does not have a +note matching '%1'.; }; @break strmatch(%qV, _note-*[v(d.default-column-delimiter)]*[v(d.default-row-delimiter)]*)={ @trigger me/tr.message=%#, moniker(%qO) has the following +notes: [itemize(iter(%qV, rest(itext(0), v(d.default-column-delimiter)), v(d.default-row-delimiter), v(d.default-row-delimiter)), v(d.default-row-delimiter))]; }; @pemit %#=ulocal(layout.note, %qO, if(t(%1), %1, xget(%qO, strcat(ulocal(f.get-key-prefix, _note-), after(first(%qV, v(d.default-column-delimiter)), -)))), %qV, %#);
...
&c.+note/delete [v(d.bc)]=$+note/del* */*:@assert t(setr(O, ulocal(f.grab-matching-settable-object, %1, %#, note)))={ @trigger me/tr.error=%#, Could not find an object you own called '%1'.; }; @assert t(setr(V, ulocal(f.get-visible-notes, %qO, %2, %#)))={ @trigger me/tr.error=%#, Could not find a note on [moniker(%qO)] called '%2'.; }; @assert eq(words(%qV, v(d.default-row-delimiter)), 1)={ @trigger me/tr.error=%#, Your delete matched multiple +notes: [itemize(iter(%qV, rest(itext(0), v(d.default-column-delimiter)), v(d.default-row-delimiter), v(d.default-row-delimiter)), v(d.default-row-delimiter))]; }; @force %#=+note/set %qO/[rest(%qV, v(d.default-column-delimiter))]=;
