...
&tr.finish-exits [v(d.bc)]=@link %2=%1; @tel %2=loc(%0); @if t(%3)={ @link %3=loc(%0); @tel %3=%1; &_exit-pair %2=%3; &_exit-pair %3=%2; @set %3=TRANSPARENT; @chown %3=%vB; }; @chown %2=%vB; @set %2=TRANSPARENT; @trigger me/tr.success=%0, The exit [ulocal(f.get-exit-name, %2)] is open and ready for use.;
...
&c.+desc [v(d.bc)]=$+desc *=*: @assert t(switch(%0, here, setr(R, loc(%#)), setr(R, %0)))={ @trigger me/tr.error=%#, Could not figure out what room you're referring to. '%0' doesn't make sense.; }; @assert cor(isdbref(%qR), t(setr(R, filter(filter.is_owner, locate(%#, %qR, *R),,, %#))))={ @trigger me/tr.error=%#, Could not find anything you own named '%0'.; }; @assert eq(words(%qR), 1)={ @trigger me/tr.error=%#, More than one thing matches '%0'.; }; @assert ulocal(filter.is_owner, %qR, %#)={ @trigger me/tr.error=%#, You must be an owner of [name(%qR)] to edit the description.; }; @assert cor(not(hasflag(%qR, INHERIT)), isstaff(%#))={ @trigger me/tr.error=%#, This thing's description cannot be set by non-staff.; }; @set %qR=desc:%1; @trigger me/tr.success=%#, cat(You have updated the description on, name(%qR) (%qR).); @trigger me/tr.remit-quiet=%qR, ulocal(f.get-name, %#) has updated the description of this room.; @if hasflag(%qR, TEMPROOM)={ @set %#=_temproom-desc:%1; }; @if cand(hastype(%qR, EXIT), hasattr(%qR, _exit-pair), hasflag(where(%qR), TEMPROOM))={ @set %#=_temproom-exit-desc:%1; }; @if cand(hastype(%qR, EXIT), hasattr(%qR, _exit-pair), hasflag(loc(%qR), TEMPROOM))={ @set %#=_temproom-entrance-desc:%1; };

@set [v(d.bc)]/c.+desc=no_parse

&c.+shortdesc [v(d.bc)]=$+shortdesc *=*:@assert t(switch(%0, here, setr(R, loc(%#)), me, setr(R, %#), setr(R, %0)))={ @trigger me/tr.error=%#, Could not figure out what you're referring to. '%0' doesn't make sense.; }; @assert cor(isdbref(%qR), t(setr(R, filter(filter.is_owner, locate(%#, %qR, himnR),,, %#))))={ @trigger me/tr.error=%#, Could not find anything you own named '%0'.; }; @assert eq(words(%qR), 1)={ @trigger me/tr.error=%#, More than one thing matches '%0'.; }; @assert lte(strlen(%1), setr(M, xget(%vD, d.max-shortdesc-length)))={ @trigger me/tr.error=%#, Your shortdesc is too long. Max length is %qM characters.; }; @assert ulocal(filter.is_owner, %qR, %#)={ @trigger me/tr.error=%#, You must be an owner of [name(%qR)] to edit the short description.; }; @assert cor(not(hasflag(%qR, INHERIT)), isstaff(%#))={ @trigger me/tr.error=%#, This thing's short description cannot be set by non-staff.; }; @set %qR=short-desc:%1; @trigger me/tr.success=%#, cat(You have updated the short-desc on, name(%qR) (%qR) to: %1); @if hasflag(%qR, TEMPROOM)={ &_temproom-short-desc %#=%1; };

@set [v(d.bc)]/c.+shortdesc=no_parse

&c.+shortdesc_alias [v(d.bc)]=$+shortdesc:@force %#=+glance;

&c.+private [v(d.bc)]=$+private*:@eval setq(R, if(t(%0), trim(%0), %L)); @assert cor(isdbref(%qR), t(setr(R, filter(filter.is_owner, locate(%#, %qR, hrR),,, %#))))={ @trigger me/tr.error=%#, Could not find any rooms you own named '%0'.; }; @assert eq(words(%qR), 1)={ @trigger me/tr.error=%#, More than one thing matches '%0'.; }; @assert ulocal(filter.is_owner, %qR, %#)={ @trigger me/tr.error=%#, You must be an owner of [name(%qR)] to set it private.; }; @set %qR=UNFINDABLE; @eval setq(X, first(trim(iter(lexits(%qR), if(strmatch(name(itext(0)), * <O>), itext(0)))))); @eval setq(E, xget(%qX, _exit-pair)); @set %qX=!TRANSPARENT; @set %qE=!TRANSPARENT; @trigger me/tr.success=%#, cat(You have marked, name(%qR) private.); @if hasflag(%qR, TEMPROOM)={ &_temproom-private %#=1; @set %qE=DARK; };


@@ TODO: Make +lock stick around through +temproom recreations!

@@ TODO: +home <location> - set your home to a location you control or have access to (that allows it)

@@ TODO: +home - list locations you control, have keys to, and your 'home' setting.

@@ TODO: +home/allow <location> and +home/deny <location> - allow players with keys to set their homes to this location

@@ &c.+home [v(d.bc)]=$+home:; @trigger me/tr.success=%#, ;

@@ TODO: +owned - all buildings you own.

@@ TODO: +owned <player> - staff command to see all buildings owned by a player.

@@ TODO: +private - set the room unfindable and the exit invisible if it's a temproom.

@@ TODO: +public - the opposite of +private

@@ TODO: Figure out how long temprooms can persist. Consider allowing them to persist for longer and letting players link themselves to them.
...
&c.+temproom [v(d.bc)]=$+temproom *:@assert ulocal(f.can-build-temproom, %#)={ @trigger me/tr.error=%#, You can't build a temproom here. [if(hasattr(%#, _current-temproom), You already have one elsewhere - you'll need to +temproom/destroy that one before you can create a new one. Type %ch+temproom/tel%cn to return to it or +temproom/destroy to destroy it.)]; }; @eval setr(N, switch(%0, *=*, first(%0, =), %0)); @assert valid(roomname, %qN)={ @trigger me/tr.error=%#, '%qN' is not a valid room name.; }; @eval setr(E, switch(%0, *=*%,*, first(rest(%0, =), %,), *=*, rest(%0, =), ulocal(f.get-exit-name, %qN))); @assert valid(exitname, %qE)={ @trigger me/tr.error=%#, '%qE' is not a valid exit name.; }; @assert ulocal(f.exit-is-not-in-use, loc(%#), %qE)={ @trigger me/tr.error=%#, One of your exit names is already in use.; }; @eval setr(O, switch(%0, *=*%,*, last(%0, %,), xget(%vD, d.default-out-exit))); @assert valid(exitname, %qO)={ @trigger me/tr.error=%#, '%qO' is not a valid exit name.; }; @trigger %vB/tr.temproom-digger=%#, %qN, %qE, %qO;

&tr.finish-temproom [v(d.bc)]=@link %2=%1; @tel %2=loc(%0); @link %3=loc(%0); @tel %3=%1; @parent %1=parent(loc(%0)); &_exit-pair %2=%3; &_exit-pair %3=%2;@chown %0=%vB; @chown %2=%vB; @chown %3=%vB; @set %2=TRANSPARENT; @set %3=TRANSPARENT; @set %1=TEMPROOM STICKY; @chown %0=%vB; &_owner-%0 %1=cat(ulocal(f.get-name, %0) created this temproom at, prettytime()); &_current-temproom %0=%1; @if hasattr(%0, _temproom-desc)={ @desc %1=[xget(%0, _temproom-desc)]; }; @if hasattr(%0, _temproom-short-desc)={ &short-desc %1=[xget(%0, _temproom-short-desc)]; }; @if hasattr(%0, _temproom-exit-desc)={ @desc %3=[xget(%0, _temproom-exit-desc)]; }; @if hasattr(%0, _temproom-entrance-desc)={ @desc %2=[xget(%0, _temproom-entrance-desc)]; }; @trigger me/tr.success=%0, The room '[name(%1)]' is built and ready for use.;
...
@@ TODO: +temproom/tel to return to a temproom you created which is still active.
