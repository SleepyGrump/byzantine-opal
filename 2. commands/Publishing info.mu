...
&layout.glance [v(d.bf)]=strcat(setq(P, filter(filter.is_connected_player, lcon(loc(%0)))), setq(O, filter(filter.isobject, lcon(loc(%0)))), divider(At a glance..., %0), %r, formattext(shortdesc(loc(%0)), 1, %0), %r, divider(, %0), %r, multicol(iter(%qP %qO, ulocal(layout.glance_line, itext(0), %0),, |), 20 4 *, 0, |, %0), %r, footer(, %0))
