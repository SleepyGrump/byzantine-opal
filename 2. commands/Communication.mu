...
&switch.msg.block [v(d.bc)]=@eval setq(P, ulocal(f.find-player, first(%1, =), %2)); @assert t(%qP)={@pemit %2=alert(MSG) Could not find player '[first(%1, =)]' (%qP).;}; @eval strcat(setq(R, rest(%1, =)), if(not(t(%qR)), setq(R, 1))); &msg-block-%qP %2=%qR; @pemit %2=alert(MSG) You have blocked [name(%qP)] from sending you messages. When [subj(%qP)] [case(subj(%qP), they, try, tries)] to message you, [subj(%qP)] will receive a message that says:%R[alert(MSG NOT SENT)] [name(%2)] has blocked you[if(match(%qR, 1), ., %bbecause: [s(%qR)])]

&switch.msg.unblock [v(d.bc)]=@eval setq(P, ulocal(f.find-player, first(%1, =), %2)); @assert t(%qP)={@pemit %2=alert(MSG) Could not find player '[first(%1, =)]' (%qP).;}; @assert hasattr(%2, msg-block-%qP)={@pemit %2=alert(MSG) You are not currently blocking [name(%qP)].}; @wipe %2/msg-block-%qP; @pemit %2=alert(MSG) You are no longer blocking [name(%qP)] from sending you messages.

&switch.msg.hide [v(d.bc)]=@eval setq(P, ulocal(f.find-player, first(%1, =), %2)); @assert t(%qP)={@pemit %2=alert(MSG) Could not find player '[first(%1, =)]' (%qP).;}; &msg-hide-%qP %2=1; @pemit %2=alert(MSG) You have hidden all messages from [name(%qP)]. Those messages will be lost forever, and [subj(%qP)] will not be notified that the message was not received.

&switch.msg.unhide [v(d.bc)]=@eval setq(P, ulocal(f.find-player, first(%1, =), %2)); @assert t(%qP)={@pemit %2=alert(MSG) Could not find player '[first(%1, =)]' (%qP).;}; @assert hasattr(%2, msg-hide-%qP)={@pemit %2=alert(MSG) You are not currently blocking [name(%qP)].}; @wipe %2/msg-hide-%qP; @pemit %2=alert(MSG) You are no longer hiding messages from [name(%qP)].
...
&switch.msg [v(d.bc)]=@break t(xget(%1, msg-receive-off))={@trigger me/tr.error=%1, You cannot send messages while your ability to receive messages is turned off. Type msg/on to enable the message system.;};@break not(t(%0))={@trigger me/switch.msg.summary=%1;};@eval setq(T, if(not(strmatch(%0, %b*)), strip(first(%0), /), xget(%1, last-msg-type)));@eval setq(L, switch(trim(%0), /* *=*, first(rest(%0), =), *=*, first(%0, =), edit(xget(%1, last-msg-target), |, %b)));@eval setq(V, trim(switch(%0, *=*, rest(%0, =), /* *, rest(%0), /*,, %0)));@break not(t(%qV))={@trigger me/switch.msg.summary=%1;};@eval strcat(setq(P, iter(%qL, ulocal(f.find-player, itext(0), %1),, |)), setq(P, setunion(%qP, %qP, |)));@eval iter(%qP, if(not(t(itext(0))), setq(E, %qE Could not find player '[if(t(%qL), extract(%qL, inum(0), 1), itext(0))]' ([itext(0)]).)), |); @break not(t(%qP))={@trigger me/tr.error=%1, You need to choose someone to send the message to. }; @break t(squish(trim(%qE)))={@trigger me/tr.error=%1, %qE;};@set %1=last-msg-target:%qP;@if t(%qT)={@set %1=last-msg-type:%qT;};@pemit %1=ulocal(layout.msg, %1, %qP, %qT, %qV);@dolist/delimit | [setdiff(setunion(%qP, %qP, |), %1, |)]={@switch/first 1=hasattr(##, msg-hide-%1), {}, hasattr(##, msg-block-%1), {@pemit %1=alert(MSG BLOCKED) [name(##)] has blocked you[if(match(setr(B, u(##/msg-block-%1)), 1), ., %bbecause: %qB)];}, and(or(t(xget(##, msg-receive-off)), not(hasflag(##, connect))), gte(words(lattr(##/_msg-*)), 50)), {@pemit %1=alert(MSG NOT DELIVERED) [name(##)] is not available and [poss(##)] message queue is full. Resend your message later.;}, or(t(xget(##, msg-receive-off)), not(hasflag(##, connect))), {&_msg-[secs()] ##=ulocal(layout.msg, %1, %qP, %qT, %qV);@pemit %1=alert(MSG DELAYED) Added message to [name(##)]'s message queue.;}, {@pemit ##=ulocal(layout.msg, %1, %qP, %qT, %qV);}}
